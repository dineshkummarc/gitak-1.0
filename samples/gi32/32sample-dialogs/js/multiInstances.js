 ...
 */
sfn.openSelectedStocks = function(list){
 try {
  // Get the area that the dialogs are to be lunched in (target)
   var areaDialogs = sfn.getServer().getJSXByName("blkDialogScrollableArea")
  ...
  for (var i = 0 ; i < selectedIds.length ; i ++){
    ...
    if ( !singlDialog ){
      ...
      sfn.createDialog(stockSymbol,areaDialogs,position);
      */
      sfn.createDialogWserFile(stockSymbol,areaDialogs,position);  
 ...
 * @ positon {int}
 */
sfn.createDialog = function(dialogName,dialogArea,position){
...
sfn.createDialogWserFile = function(dialogName,dialogArea,position){
