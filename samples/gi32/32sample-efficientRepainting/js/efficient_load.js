...
effLoad.g_i;
/*
 * Loads 20 successive dialogs into the app: either inefficiently (repaint) or efficiently (insertHTML with an incremented delay)
...
effLoad.loadDialogs = function(objParent,bEfficient,intCount) {
  ...
  if(bEfficient) {
    ...
    for(var i=0;i<intCount;i++) {
      //PERFORMANCE GAIN 1): load the new dialog using an incremented delay (100 milliseconds)
      window.setTimeout(function() {
        effLoad.loadOneDialogEfficient(objParent,true);
      },i*100); //==> use an 'incremented delay'
    }
  } else {
    for(var i=0;i<intCount;i++) {
...
/*
 * Loads a single dialog into the application using 'insertHTML'
...
effLoad.loadOneDialogEfficient = function (objParent,bTile) {
  //PERFORMANCE GAIN 2): load the new dialog using insertHTML
