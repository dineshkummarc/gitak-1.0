...
effAdopt.activeContainer = 'dialog'
  ...
  effAdopt.loadOneItem = function() {
    ...
    var viewtype = jsx3.gui.RadioButton.getValue("group1");
    //get parent container to add new item to
    var objParent = eg.effRepainting.getServer().getJSXByName("my" + viewtype + "container");
  ...
  effAdopt.transferAllItems = function(viewtype) {
    ...
    var objOldParent = eg.effRepainting.getServer().getJSXByName("my" +  effAdopt.activeContainer + "container");
    //get new container
    var objNewParent = eg.effRepainting.getServer().getJSXByName("my" + viewtype + "container");
    //loop through all children in the old container
    // var intStartIndex = (prevview == "dialog") ? 1 : 0;
    ...
    for(var i=intStartIndex;i<maxLen;i++) {
      //effAdopt.transferItem(objOldParent,objNewParent,prevview,viewtype,i);
  ...
  effAdopt.removeAllItems = function(viewtype) {
    ...
    if(viewtype == null) viewtype = jsx3.gui.RadioButton.getValue("group1");
    ...
    for(var i=maxLen;i>=0;i--) {
      ...
      if(!objChild.isInstanceOf("jsx3.gui.Splitter")) objParent.removeChild(objChild);
    }
  }
})